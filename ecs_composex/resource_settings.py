...
def generate_export_strings(res_name, attribute):
    ...
    if isinstance(attribute, str):
        cfn_string = (
            f"${{{ROOT_STACK_NAME_T}}}{DELIM}{res_name}{DELIM}{attribute}"
        )
...
def generate_resource_permissions(resource_name, policies, attribute, arn=None):
    ...
    for a_type in policies:
        ...
        policy_doc["Resource"] = generate_export_strings(resource_name, attribute) if not arn else arn
...
def generate_resource_envvars(resource_name, resource, attribute, arn=None):
    ...
    """
    env_names = []
    export_strings = generate_export_strings(resource_name, attribute) if not arn else arn
