...
def add_load_balancer(service, ports, **kwargs):
    ...
    alb_sg = None
    eips = []
    if service.config.is_public and service.config.lb_type == "network":
        eips = add_public_ips(service.template, service.service_name, kwargs["AwsAzs"])

    no_value = Ref("AWS::NoValue")
    public_mapping = define_public_mapping(eips, kwargs["AwsAzs"])
    if ports and service.config.lb_type == "application":
        alb_sg = add_alb_sg(service.template, ports, service.config.is_public)
        add_lb_to_service_ingress(service, alb_sg, SG_T)
        lb_sg = [Ref(alb_sg)]
    else:
        lb_sg = no_value

    loadbalancer = LoadBalancer(
        f"Microservice{service.config.lb_type.title()}LB",
        template=service.template,
        Scheme="internet-facing" if service.config.is_public else "internal",
        LoadBalancerAttributes=[
            LoadBalancerAttributes(
                Key="load_balancing.cross_zone.enabled", Value="true"
            )
        ]
        if service.config.lb_type == "network"
        else no_value,
        ...
        if service.config.is_public and service.config.lb_type == "application"
        else no_value,
        Type=service.config.lb_type,
        Tags=Tags(
            {
...
def add_service_load_balancer(service, **kwargs):
    ...
    tgt_groups = []
    depends_on = []
    curated_ports = [port["target"] for port in service.config.ports]
    service_lb = add_load_balancer(
        service, curated_ports, **kwargs
    )
...
def define_service_load_balancing(service, **kwargs):
    """Function to add all the resources for when using ALB or NLB

    :return: service_lb
    :rtype: tuple
    """
    LOG.debug(f"Adding LB for service {service.service_name}")
    return add_service_load_balancer(service, **kwargs)
